...
//                        System.out.println("SUM: epsilon * (" + jointOutcome + ", " + eS + ") >=  (1-epsilon) * " + stringSum);
        ...
        // adding additional constraints to check the robustness of given strategy
        for (BitSet c : strategy.keySet()) {
            ArrayList<Integer> varList = getValuesWithMatchingFirstBitSet(epsilonCeVarMap, c);
            GRBLinExpr expr = new GRBLinExpr();
            for (Integer var : varList) {
                expr.addTerm(1.0, vars[var]);
            }
            model.addConstr(expr, GRB.EQUAL, strategy.get(c), "strategyConstraint");
        }
